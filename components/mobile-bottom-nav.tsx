...
export default function MobileBottomNav() {
  ...
  const [moreSelectedIndex, setMoreSelectedIndex] = useState<number | null>(
    null
  );
      ...
      try {
        ...
        const parseReleased = (
          ...
          if (m2) {
            ...
            if (mm)
              return { y: parseInt(m2[3], 10), m: mm, d: parseInt(m2[1], 10) };
          }
          const d = new Date(s);
          if (!isNaN(d.getTime()))
        ...
        const filtered = items
          ...
          .sort((a: any, b: any) => {
            const da = parseReleased(
              (a?.details || a || {}).release_date ??
                (a?.details || a || {}).released
            );
            const db = parseReleased(
              (b?.details || b || {}).release_date ??
                (b?.details || b || {}).released
            );
        ...
        const currKey = JSON.stringify(
          mapped.map((m: any) => `${m.id}:${m.released ?? ""}`)
        );
      ...
      try {
        ...
        const mapped = filtered.slice(0, maxCount).map((it: any) => {
          ...
          return {
            ...
            ratingLabel: toRatingLabel(
              d.rating_kp ?? d.rating ?? it.rating ?? null
            ),
            href: d.id ? `/movie/${String(d.id)}` : null,
          };
        });
        try {
          const idsString = mapped
            .map((m: any) => String(m.id))
            .filter(Boolean)
            .join(",");
          if (idsString) {
            const resOv = await fetch(
        ...
        try {
          ...
          for (const mid of ids) {
            try {
              ...
              const getRating = (
                ...
                if (Number.isNaN(value) || value === 0 || label === "0.0")
                  return null;
                return { value, label };
              };
              const yrRaw =
                (d2 as any).year ??
                (d2 as any).released ??
                (d2 as any).release_year ??
                (d2 as any).releaseDate;
              const year = (() => {
              ...
              const dateStr =
                (d2 as any).release_date ?? (d2 as any).released ?? null;
              const ratingKP = getRating(
                (d2 as any).rating_kp ?? (d2 as any).rating
              );
              const ratingIMDb = getRating((d2 as any).rating_imdb);
              const description2 =
                (d2 as any).about ??
                (d2 as any).plot ??
                (d2 as any).description ??
                null;
              ...
              const ratingLabelFromList =
                mapped.find((m: any) => String(m.id) === mid)?.ratingLabel ??
                null;
              const ratingLabel =
                ratingLabelFromList ??
                ratingKP?.label ??
              ...
              const ratingValue =
                ratingLabel != null
                  ? parseFloat(String(ratingLabel))
                  : ratingKP?.value ?? ratingIMDb?.value ?? null;
          ...
          const combined = mapped.map((p: any) => ({
            ...p,
            meta: detailMap[p.id] || null,
            description:
              (detailMap[p.id]?.description ?? p.description) || null,
          }));
    ...
    const fetchSerials = async () => {
      ...
      try {
        ...
        const res = await fetch(
          "https://api.vokino.pro/v2/list?sort=updatings&type=serial",
          {
            headers: { Accept: "application/json" },
            cache: "no-store",
            signal: controller.signal,
        ...
        const mapped = items
          .map((it: any) => {
            ...
            const ratingLabel = toRatingLabel(
              d.rating_kp ?? d.rating ?? it.rating ?? null
            );
        ...
        try {
          ...
          for (const mid of ids) {
            try {
              ...
              const fromList =
                top.find((m: any) => String(m.id) === mid)?.ratingLabel ?? null;
              const ratingLabel =
                fromList ?? ratingKP?.label ?? ratingIMDb?.label ?? null;
              const ratingValue =
                ratingLabel != null
                  ? parseFloat(String(ratingLabel))
                  : ratingKP?.value ?? ratingIMDb?.value ?? null;
  ...
  return (
    <>
        ...
        <div className="pointer-events-auto flex items-center justify-around bg-zinc-900/90 px-2 py-2 pb-[calc(env(safe-area-inset-bottom,0px)+8px)] border-t border-zinc-800/70 backdrop-blur-sm shadow-[0_16px_36px_rgba(0,0,0,0.55)]">
          ...
          >
            <IconHeart
              className="w-5 h-5"
              stroke={1.6}
              fill={isFavoritesMode ? "currentColor" : "none"}
            />
          </button>
          <button
            type="button"
      ...
      <Drawer
        ...
        onOpenChange={(o) => {
          setIsMoreOpen(o);
          if (!o) setMoreSelectedIndex(null);
        }}
      >
        <DrawerContent>
          <DrawerHeader>
          ...
          <div className="p-4">
            <div className="grid grid-cols-3 gap-2">
              {CATEGORIES.filter(
                (c) => !["4K UHD", "Фильмы", "Сериалы"].includes(c.title)
              ).map((cat, idx) => (
                <button
                  key={idx}
                  onClick={() => {
          ...
          <div className="p-3 flex-1 min-h-0 overflow-y-auto">
            <Tabs value={bellTab} onValueChange={(v) => setBellTab(v as any)}>
              <TabsList className="bg-zinc-800/70 rounded-full px-1.5 py-1">
                <TabsTrigger value="premieres" className="rounded-full px-3">
                  Премьеры
                </TabsTrigger>
                <TabsTrigger value="serials" className="rounded-full px-3">
                  Обновление сериалов
                </TabsTrigger>
              </TabsList>
                ...
                <div className="flex flex-col gap-2">
                  {premieresLoading ? (
                    ...
                    <div className="text-sm text-zinc-400">
                      За последний месяц премьер нет
                    </div>
                  ) : (
                    premieres.map((p, idx) => (
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              <div className="min-w-0 flex items-center gap-2">
                                <div className="text-[13px] md:text-sm font-semibold text-zinc-100 truncate">
                                  {p.title
                                    ? `${p.meta?.typeLabel ?? "Премьера"}: ${
                                        p.title
                                      }`
                                    : "Премьера"}
                                </div>
                            ...
                            <div className="mt-0.5 flex items-center gap-2 text-[12px] text-zinc-400">
                              {(() => {
                                ...
                                if (rv != null || rl) {
                                  return (
                                    <span
                                      className={[
                                        "shrink-0 inline-flex items-center rounded-full px-1.5 py-[1px] text-[12px] text-white",
                                        ratingBgColor(rv ?? rl),
                                      ].join(" ")}
                                    >
                                      {rl
                                        ? formatRatingLabel(rl)
                                        : formatRatingLabel(rv)}
                                    </span>
                                  );
                                }
                                return null;
                              ...
                              {(() => {
                                const parts: string[] = [];
                                const country = p.meta?.countryLabel
                                  ? String(p.meta.countryLabel)
                                  : "";
              ...
              <TabsContent value="serials" className="mt-3">
                <div className="flex flex-col gap-2">
                  {serialsLoading ? (
                    ...
                    <div className="text-sm text-zinc-400">
                      Нет обновлений сериалов с рейтингом
                    </div>
                  ) : (
                    serials.map((p, idx) => (
                      <a
                        key={idx}
                        href={p.id ? `/movie/${p.id}` : undefined}
                        className="block"
                      >
                        <div className="bg-zinc-900/70 rounded-[10px] p-2 flex items-start gap-2">
                          <div className="relative aspect-[2/3] w-[64px] rounded-[6px] overflow-hidden bg-zinc-800/60">
                            ...
                            <div className="mt-0.5 flex items-center gap-2 text-[12px] text-zinc-400">
                              {(() => {
                                const rv =
                                  p?.meta?.ratingValue ??
                                  (p.ratingLabel
                                    ? parseFloat(String(p.ratingLabel))
                                    : null);
                                const rl =
                                  p?.meta?.ratingLabel ?? p.ratingLabel;
                                if (rv != null || rl) {
                                  return (
                                    <span
                                      className={[
